...
function getCoordInputs() {
    ...
    return {
        x: clampCoord(xInput ? xInput.value : 0, BOARD_WIDTH - 1),
        y: clampCoord(yInput ? yInput.value : 0, BOARD_HEIGHT - 1)
    };
}

function setCoordInputs(x, y) {
    const xInput = document.getElementById('coordX');
    const yInput = document.getElementById('coordY');
    if (xInput) xInput.value = clampCoord(x, BOARD_WIDTH - 1);
    if (yInput) yInput.value = clampCoord(y, BOARD_HEIGHT - 1);
}

window.nudgeCoords = function(dx, dy) {
    const { x, y } = getCoordInputs();
    setCoordInputs(x + dx, y + dy);
};

window.placeSelectedFromPanel = function() {
    if (!selectedPiece) {
    ...
    if (canPlacePiece(x, y)) {
        placePiece(x, y);
        showStatus(`Placed at (${x}, ${y}).`, 'success');
    } else {
        showStatus(`Cannot place at (${x}, ${y}).`, 'error');
    }
};
...
function setupKeyboardShortcuts() {
    document.addEventListener('keydown', (evt) => {
        ...
        if (evt.key === 'r' || evt.key === 'R') {
            console.log('[Shortcut] Rotate (R)');
            rotateSelectedPiece();
            evt.preventDefault();
        } else if (evt.key === 'f' || evt.key === 'F') {
            console.log('[Shortcut] Flip (F)');
            flipSelectedPiece();
            evt.preventDefault();
        } else if (evt.key === 'Backspace') {
            console.log('[Shortcut] Undo (Backspace)');
            undoLastMove();
            evt.preventDefault();
        } else if (evt.key === 'c' || evt.key === 'C') {
            console.log('[Shortcut] Clear (C)');
            clearBoard();
            evt.preventDefault();
        }
    });
